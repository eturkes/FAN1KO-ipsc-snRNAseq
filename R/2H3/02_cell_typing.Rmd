...
### Assigning Seurat Clusters

```{r}
rds <- file.path(cache_dir, "tran_singleR_clusters.rds")
if (file.exists(rds)) {
  predictions <- readRDS(rds)
} else {
  predictions <- SingleR(
    GetAssayData(seurat, assay = "RNA"), ref, ref$cellType,
    clusters = seurat$seurat_clusters, BPPARAM = MulticoreParam(detectCores())
  )
  saveRDS(predictions, rds)
}
...
names(predictions$labels) <- levels(seurat)
seurat <- RenameIdents(seurat, predictions$labels)
seurat$tran_singleR_clusters <- seurat@active.ident
red_dim_plot(seurat, "umap1", "umap2", "tran_singleR_clusters", "cat")
```
...
### Merged w/ Seurat Cluster Assignment

```{r}
rds <- file.path(cache_dir, "tran_singleR_merged_clusters.rds")
if (file.exists(rds)) {
  predictions <- readRDS(rds)
} else {
  predictions <- SingleR(
    GetAssayData(seurat, assay = "RNA"), ref, ref$cellType_merged,
    clusters = seurat$seurat_clusters, BPPARAM = MulticoreParam(detectCores())
  )
  saveRDS(predictions, rds)
}
...
names(predictions$labels) <- levels(seurat)
seurat <- RenameIdents(seurat, predictions$labels)
seurat$tran_singleR_merged_clusters <- seurat@active.ident
red_dim_plot(seurat, "umap1", "umap2", "tran_singleR_merged_clusters", "cat")
```
