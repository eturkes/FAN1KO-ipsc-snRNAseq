...
packages <- c("conflicted", "Seurat", "dplyr", "ggrepel", "ggplot2", "GSVA", "patchwork")
  ...
  if (any(smith_markers[[i]] %in% rownames(seurat))) {
    print(
      FeaturePlot(seurat, smith_markers[[i]], order = TRUE, cols = c("lightgrey", "red")) +
        plot_annotation(
          names(smith_markers)[i], theme = theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold"))
        )
    )
  }
}
...
for (i in seq_along(tran_markers)) {
  if (any(tran_markers[[i]] %in% rownames(seurat))) {
    print( # Only plot first 4 genes.
      FeaturePlot(seurat, tran_markers[[i]][1:4], order = TRUE, cols = c("lightgrey", "red")) +
        plot_annotation(
          names(tran_markers)[i], theme = theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold"))
        )
    )
  }
}
...
for (i in seq(ceiling(nrow(seurat) / 4))) { # Find number of rounds needed to loop through all gene sets.
  ...
  print(FeaturePlot(seurat, rownames(seurat)[idx_start:idx_end], order = TRUE, cols = c("lightgrey", "red")))
}
# -----------------------------------------------------
